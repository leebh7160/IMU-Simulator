...
print("ROS-Style ESKF Implementation with Railway Route Projection")
print("=" * 60)
...
class ImuGpsLocalizer:
    def __init__(self, acc_noise=0.5, gyro_noise=0.01,
        ...
        try:
            ...
            print(f"Loaded {len(self.rail_nodes)} railway nodes for route projection")
        except:
            self.rail_nodes = None
            print("No railway_nodes.csv found - proceeding without route projection")
...
if __name__ == "__main__":
    ...
    if eskf_display:
        fig.add_trace(go.Scattermapbox(
            ...
            name='ESKF Path (Route-Projected)' if localizer.rail_nodes is not None else 'ESKF Path'
        ))
    if eskf_path:
        fig.add_trace(go.Scattermapbox(
            mode="markers+text",
    ...
    fig.update_layout(
        ...
        title=f"ESKF with Railway Route Projection<br><sub>GPS: {gps_update_count}, IMU: {imu_update_count}, Rail Nodes: {len(rail_path)}</sub>",
        height=800,
        showlegend=True
    )
